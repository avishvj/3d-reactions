...
class Temp(InMemoryDataset):
    ...
    def __init__(self, root, geo_file, transform=None, pre_transform=None, dataset_type="individual"):
        super(Temp, self).__init__(root, transform, pre_transform)
        self.geo_file = geo_file        
        self.dataset_type = dataset_type

        if dataset_type == 'individual':
            self.data, self.slices = torch.load(self.processed_paths[TempEnum[geo_file].value]) # ind
        elif dataset_type == 'concat':
            self.data, self.slices = torch.load(self.processed_paths[TempConcatEnum[geo_file].value]) # concat
    ...
    def process_geometry_file(self, geometry_file, list=None):
        ...
        for i, mol in enumerate(tqdm(geometries)):
            ...
            if list is not None: # if want to update list
                list.append(data)
            else:
